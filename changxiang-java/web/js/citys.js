...
cityArr[30]=['海南省','三沙市','海口市','三亚市','五指山市','琼海市','儋州市','文昌市','万宁市','东方市','定安县','屯昌县','澄迈县','临高县','白沙黎族自治县','昌江黎族自治县','乐东黎族自治县','陵水黎族自治县','保亭黎族苗族自治县','琼中黎族苗族自治县'];
...
cityArr[33]=['花地玛堂','圣安多尼堂','大堂','望德堂','风顺堂','嘉模堂','圣方济各堂','路氹城'];
cityArr[34]=['主岛','黄尾屿','南小岛','北小岛','赤尾屿','北屿','南屿','飞濑屿'];
var province=document.getElementById('province'); 
var city=document.getElementById('city');
for ( var i = 0; i < provinceArr.length; i++) {//生成省份  
	var provinceOption=document.createElement('option');
	provinceOption.value=provinceOption.text=provinceArr[i];
	province.options.add(provinceOption);
}
function showChild(){
	var she=document.getElementById('province').value;
	var count=cityArr.length;
	city.length=1;
	for(var i=0;i<count;i++){
		if (she==provinceArr[i]){
			for(var j=0;j<cityArr[i].length;j++){
				var childOption=document.createElement('option');
				childOption.value=childOption.text =cityArr[i][j];//ie不支持option对象的value,所以加childOption.text
				city.options.add(childOption);
			}  
		}
	}
}
if (province.addEventListener){  
	province.addEventListener('change',function(){showChild();},false);
}else if(province.attachEvent){
	province.attachEvent('onchange',function(){showChild();});
}