...
local={
	...
	NavInit:function(u,m){
		...
		if(u.length>0){
			...
			location.href='/login.html';
			h+='欢迎来到鼎盛！<a onclick="local.login()" href="javascript:void(0);">请登录</a> <a target="_blank" href="/R.html">免费注册</a></div>';
		}
		$('#topNavLeft').html(h);
	},
	ScrollImg:function(){//图片轮询
		...
		$('#cpIndexAdBox div.ctrol a').mouseover(function(){
			...
			$('#cpIndexAdBox .adList').animate({left:'-'+CurrentIndex*1920+'px'},0);
		});
		var p=function(){
...
Show={
	...
	LotName:function(s){//彩种名字
		...
		switch(s){
			...
			case 'Jxssc':r='天津时时彩';break;
			case 'Hnssc':r='河内五分彩';break;
			case 'Ynssc':r='印尼五分彩';break;
			case 'Txffc':r='腾讯分分彩';break;
	...
	LogoName:function(s){//logo和彩种
		...
		switch(s){
			...
			case 'Jxssc':r='<span class="cz_logo cz_jxssc"></span><h1>天津时时彩</h1>';break;
			...
			case 'Txffc':r='<span class="cz_logo cz_ssc"></span><h1>腾讯分分彩</h1>';break;
	...
	LotCode:function(s){//认购内容显示
		...
		for(var i = 0;i < codes.length;i++){
			...
			if(code.length&&code.length==4){
				...
				r+='<tr><td>'+nm+'&nbsp;&nbsp;&nbsp;'+code[1]+'</td><td>'+code[2]+'</td></tr>';
			}else{
				r+='<tr><td>['+nm+']&nbsp;&nbsp;&nbsp;'+code[1]+'</td><td>'+code[2]+'</td></tr>';
			}
		}
...
//			var code = codes[i].split(':');
//			var nm = PName[code[0]];
//			if(code.length&&code.length==4&&code[3]){
//				var lot_type = code[3];
//				if(lot_type=='3'){
//					nm+="[后三]";
//				}
	...
	LotCode2:function(s){//认购内容显示
		...
		for(var i = 0;i < codes.length;i++){
			...
			if(code.length&&code.length==4&&code[3]){
				...
				var input=code[1].split(',');
				for(var j = 0;j < input.length;j++){
					r2+='<input style="width:80px;border-radius:4px;box-sizing:border-box;"  name =\'kk\'  value='+input[j]+'>'+'\n'
				}
				r2+='&nbsp;&nbsp;&nbsp;';
				r+='<div style="margin-bottom:10px">'+'['+nm+']&nbsp;&nbsp;&nbsp;'+r2+' 注数：'+code[2]+'<br>'+'</div>';
				r2='';
			}else{
				//r+='['+nm+']&nbsp;&nbsp;&nbsp;'+code[1]+' 注数：'+code[2]+'<br>';
				var input=code[1].split(',');
