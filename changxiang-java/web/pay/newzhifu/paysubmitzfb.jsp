...
<%
	...
	String AdviceURL ="http://520babe.com/pay/newzhifu/notifyzfb.jsp";   //[必填]异步通知地址
	...
	if (money>=50&&null!=pa_MP){
	    ...
	    if (null!=find){
	     int uid = find.getUser_id();
			...
			String asynNotifyUrl = "http://www.520babe.com/pay/newzhifu/notifyzfb.jsp";//支付异步返回地址
			String synNotifyUrl = "http://www.520babe.com/pay/newzhifu/notifyzfb.jsp";//支付同步返回地址
			String signType = "MD5";//默认填写：0008（M）
			String merId =  "00000000518771";//商户ID
			String prdOrdNo =  BillNo;//  商品订单号（M） 
			String payMode =  "00021";//支付方式， 00022-微信扫码(固定值)（M）
			String receivableType = "T01";//到账类型（M）
