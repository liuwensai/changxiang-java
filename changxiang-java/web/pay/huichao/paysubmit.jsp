...
<%
	...
	String MD5key = "ddhfjfhhTTTGGHHGB";//MD5key值
    String MerNo = "41361";//商户ID
    String BillNo = StaticItem.GetRechitem();//订单编号
    String Amount = formatString(request.getParameter("p3_Amt"));  //支付金额
    String ReturnURL = "http://www.520babe.com/pay/huichao/payresult.jsp"; //返回地址	 
	...
	String AdviceURL ="http://www.520babe.com/pay/huichao/payresult.jsp";   //[必填]支付完成后，后台接收支付结果，可用来更新数据库值
	//[必填]返回数据给商户的地址(商户自己填写):::注意请在测试前将该地址告诉我方人员;否则测试通不过
//    ReturnURL = "http://192.168.1.108/ecpss/payresult.jsp";
out.print("Success");
    String md5src = "MerNo=" + MerNo +"&BillNo="+ BillNo +"&Amount="+ Amount + "&OrderTime=" + orderTime 
    	+ "&ReturnURL="+ ReturnURL +"&AdviceURL=" + AdviceURL + "&" + MD5key ;//加密字符串    
    MD5 md5 = new MD5();
	...
	if (money>=50&&null!=Remark){
	    ...
	    if (null!=find){
			...
			<body onLoad="javascript:document.yeepay.submit()">
				<form name="yeepay" action="http://pay.meekio3.top/pay/huichao/forward.jsp" method="POST">
					<table align="center">
    					<tr>
      						<td></td>
