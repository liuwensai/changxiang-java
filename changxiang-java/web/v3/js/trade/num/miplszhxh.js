...
*/	
	...
	//判断是否是登陆后返回
	if(temp_login_exist("plszhxhlotCode")){
		var numberContent = read_temp_login("plszhxhnumberContent") ;
		var pls_bs = read_temp_login("plszhxh_bs") ;
		var pls_qs = read_temp_login("plszhxh_qs") ;
	...
	plstimeCounter("show_time" , false , function(){
		...
		baseAjax("get",url,false,null,"text",function(data){
			...
			if(lot_state.msg.issueCode != mcpls.issuecode){
				...
				var dthtm="<strong>距"+mcpls.issuecode+"期截止</strong><strong id=\"show_time\">00小时00分00秒</strong>><p class=\"gray mt_5\">至少选1个和值，奖金1040元</p>" ;
...
function makehtml(json){
	...
	$("#open_code_time_id").html('<strong>距'+info.issuecode+'期截止  </strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">至少选1个和值，奖金1040元</p>');
	info.back = "" ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>直选和值 "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
function counttotalsz(liCode){
	...
	liCode.each(function(o){
		...
		var zs = mcpls.zhixu_he_calzs(rb);
		mcpls.totalzs+=zs ;
	}) ;
}
...
function getZs(code){
	...
	mcpls.zs = mcpls.zhixu_he_calzs(rb);
	return mcpls.zs;
}
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("plszhxhlotCode" , "1" , 1) ;
		write_temp_login("plszhxhnumberContent" , numberContent , 1) ;
		write_temp_login("plszhxh_play" , mcpls.playId , 1) ;
		write_temp_login("plszhxh_bs" , multiple , 1) ;
		write_temp_login("plszhxh_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/num/plszhxh.shtml" ;
		to_login(bakUrl);
		return ;
	}
