...
*/	
	...
	//判断是否是登陆后返回
	if(temp_login_exist("plszxhlotCode")){
		var numberContent = read_temp_login("plszxhnumberContent") ;
		var pls_bs = read_temp_login("plszxh_bs") ;
		var pls_qs = read_temp_login("plszxh_qs") ;
...
function presentSelNum(){
	...
	if(selZSBalls.length<1){
		...
		var s = mcpls.zx_he_calzs(selZSBalls) ;
		mcpls.zs = s ;
		mcpls.amt = s * 2 ;
	...
	plstimeCounter("show_time" , false , function(){
		...
		baseAjax("get",url,false,null,"text",function(data){
			...
			if(lot_state.msg.issueCode != mcpls.issuecode){
				...
				var dthtm="<strong>距"+mcpls.issuecode+"期截止</strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">至少选1个和值，组三奖金346元，组六奖金173元</p>" ;
...
function makehtml(json){
	...
	$("#open_code_time_id").html('<strong>距'+info.issuecode+'期截止  </strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">至少选1个和值，组三奖金346元，组六奖金173元</p>');
	info.back = "" ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>组选和值  "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("plszxhlotCode" , "1" , 1) ;
		write_temp_login("plszxhnumberContent" , numberContent , 1) ;
		write_temp_login("plszxh_play" , mcpls.playId , 1) ;
		write_temp_login("plszxh_bs" , multiple , 1) ;
		write_temp_login("plszxh_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/num/plszxh.shtml" ;
		to_login(bakUrl);
		return ;
	}
