...
function initBallObj(){
	...
	$("i[name='shiball']").each(function(i,o){
		shiBall[i] = $(o) ;
		shiBall[i].click(function(){
			var thisp = $(this) ;
			var classValue= thisp.context.getAttribute("class") ;
			if(classValue=="i-redball"){
				thisp.context.setAttribute("class","tored") ;
				ballObje.shiNum = ballObje.shiNum + 1 ;
			} else if(classValue=="tored"){
				thisp.context.setAttribute("class","i-redball") ;
				ballObje.shiNum = ballObje.shiNum - 1 ;
			}
	...
	$("i[name='geball']").each(function(i,o){
		geBall[i] = $(o) ;
		geBall[i].click(function(){
			var thisp = $(this) ;
			var classValue= thisp.context.getAttribute("class") ;
			if(classValue=="i-redball"){
				thisp.context.setAttribute("class","tored") ;
	...
	ballObje.baiball = baiBall ;
	ballObje.shiball = shiBall ;
	ballObje.geball = geBall ;
}
...
$(document).ready(function(){
	...
	$("#tobuy").click(function(){		
		...
		var bai = mcpls.baiball.join(",") ;
		var shi = mcpls.shiball.join(",") ;
		var ge = mcpls.geball.join(",") ;
		
		mcpls.listCode.push(bai+"|"+shi+"|"+ge) ;
		showselq();
	}) ;
...
function presentSelNum(){
	...
	for (var cc = 0; cc < 10; cc++) {
		//redthisp = selRed[cc] ;
		baithisp = ballObje.baiball[cc].context ;
		baiCls =  baithisp.getAttribute("class") ;
		baiv = baithisp.getAttribute("index") ;
		if(baiCls=="tored"){
			selBaiBalls.push(baiv) ;
		}
		
		shithisp = ballObje.shiball[cc].context ;
		shiCls =  shithisp.getAttribute("class") ;
		shiv = shithisp.getAttribute("index") ;
		if(shiCls=="tored"){
			selShiBalls.push(shiv) ;
		}
		
		gethisp = ballObje.geball[cc].context ;
		geCls =  gethisp.getAttribute("class") ;
		gev = gethisp.getAttribute("index") ;
		if(geCls=="tored"){
			selGeBalls.push(gev) ;
		}
	}
	
	if(selBaiBalls.length <1 || selShiBalls.length <1 || selGeBalls.length < 1){
		mcpls.zs = 0 ;
		mcpls.baiball=[] ;
		mcpls.shiball=[] ;
		mcpls.geball=[] ;
		...
		var s = mcpls.calZS(selGeBalls, selShiBalls, selBaiBalls) ;
		mcpls.zs = s ;
		mcpls.amt = s * 2 ;
		mcpls.baiball=selBaiBalls ;
		mcpls.shiball=selShiBalls ;
		mcpls.geball=selGeBalls ;
	...
	plstimeCounter("show_time" , false , function(){
		...
		baseAjax("get",url,false,null,"text",function(data){
			...
			if(lot_state.msg.issueCode != mcpls.issuecode){
				...
				var dthtm="<strong>距"+mcpls.issuecode+"期截止</strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">每位至少选1个号，奖金1040元</p>" ;
...
function initdata(){
	...
	var danObj ;
	for (var cc = 0; cc < 10; cc++) {
		danObj = ballObje.baiball[cc].context ;
		danObj.setAttribute("class","i-redball");
		
		danObj = ballObje.shiball[cc].context ;
		danObj.setAttribute("class","i-redball");
		
		danObj = ballObje.geball[cc].context ;
		danObj.setAttribute("class","i-redball");
	}
...
function makehtml(json){
	...
	$("#open_code_time_id").html('<strong>距'+info.issuecode+'期截止  </strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">每位至少选1个号，奖金1040元</p>');
	info.back = "" ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>直选 "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
function getZs(code){
	...
	mcpls.zs = mcpls.calZS(ge, shi, bai);
	return mcpls.zs;
}
...
function showRandom(){
	...
	var baiRan = getRedRandom();
	var shiRan = getRedRandom() ;
	var geRan = getRedRandom() ;
	
	ballObje.baiball[baiRan[0]].context.setAttribute("class" , "tored") ;
	mcpls.baiball.push(baiRan[0]) ;
	
	ballObje.shiball[shiRan[0]].context.setAttribute("class" , "tored") ;
	mcpls.shiball.push(shiRan[0]) ;
	
	ballObje.geball[geRan[0]].context.setAttribute("class" , "tored") ;
	mcpls.geball.push(geRan[0]) ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		//登录
		var bakUrl = "/v3shtml/trade/num/plszhx.shtml" ;
		to_login(bakUrl);
		return ;
	}
...
function dobuy(){
	var numberContent = mcpls.listCode.join("$").replace(/,/g,"").replace(/[|]/g,",");
...
function doTempBuy(argJSON){
	...
	var userBalacne = argJSON.allMoney;
	 var numberObject = mcpls.listCode.join("$").replace(/,/g,"").replace(/[|]/g,",");
