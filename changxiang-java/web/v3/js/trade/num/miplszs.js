...
function initBallObj(){
	...
	$("i[name='zsball']").each(function(i,o){
		zsBall[i] = $(o) ;
...
*/	
	...
	//判断是否是登陆后返回
	if(temp_login_exist("plszslotCode")){
		var numberContent = read_temp_login("plszsnumberContent") ;
		var pls_bs = read_temp_login("plszs_bs") ;
		var pls_qs = read_temp_login("plszs_qs") ;
	...
	plstimeCounter("show_time" , false , function(){
		...
		baseAjax("get",url,false,null,"text",function(data){
			...
			if(lot_state.msg.issueCode != mcpls.issuecode){
				...
				var dthtm="<strong>距"+mcpls.issuecode+"期截止</strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">至少选2个号，奖金346元</p>" ;
...
function makehtml(json){
	...
	$("#open_code_time_id").html('<strong>距'+info.issuecode+'期截止  </strong><strong id=\"show_time\">00小时00分00秒</strong><p class=\"gray mt_5\">至少选2个号，奖金346元</p>');
	info.back = "" ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>组三 "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("plszslotCode" , "1" , 1) ;
		write_temp_login("plszsnumberContent" , numberContent , 1) ;
		write_temp_login("plszs_play" , mcpls.playId , 1) ;
		write_temp_login("plszs_bs" , multiple , 1) ;
		write_temp_login("plszs_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/num/plszs.shtml" ;
		to_login(bakUrl);
		return ;
	}
