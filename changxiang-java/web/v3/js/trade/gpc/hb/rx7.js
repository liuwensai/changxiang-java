...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("hbrx7lotCode")){
		var numberContent = read_temp_login("hbrx7numberContent") ;
		var pls_bs = read_temp_login("hbrx7_bs") ;
		var pls_qs = read_temp_login("hbrx7_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("hbrx7lotCode" , "20" , 1) ;
		write_temp_login("hbrx7numberContent" , numberContent , 1) ;
		write_temp_login("hbrx7_play" , gpc.playId , 1) ;
		write_temp_login("hbrx7_bs" , multiple , 1) ;
		write_temp_login("hbrx7_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/hb/rx7.shtml" ;
		to_login(bakUrl);
		return ;
	}
