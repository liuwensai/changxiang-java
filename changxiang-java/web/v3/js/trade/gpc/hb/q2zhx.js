...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("hbq2zhxlotCode")){
		var numberContent = read_temp_login("hbq2zhxnumberContent") ;
		var pls_bs = read_temp_login("hbq2zhx_bs") ;
		var pls_qs = read_temp_login("hbq2zhx_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("hbq2zhxlotCode" , "1" , 1) ;
		write_temp_login("hbq2zhxnumberContent" , numberContent , 1) ;
		write_temp_login("hbq2zhx_play" , gpc.playId , 1) ;
		write_temp_login("hbq2zhx_bs" , multiple , 1) ;
		write_temp_login("hbq2zhx_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/hb/q2zhx.shtml" ;
		to_login(bakUrl);
		return ;
	}
