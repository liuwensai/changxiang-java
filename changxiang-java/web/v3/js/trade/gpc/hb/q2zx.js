...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("hbq2zxlotCode")){
		var numberContent = read_temp_login("hbq2zxnumberContent") ;
		var pls_bs = read_temp_login("hbq2zx_bs") ;
		var pls_qs = read_temp_login("hbq2zx_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("hbq2zxlotCode" , "20" , 1) ;
		write_temp_login("hbq2zxnumberContent" , numberContent , 1) ;
		write_temp_login("hbq2zx_play" , gpc.playId , 1) ;
		write_temp_login("hbq2zx_bs" , multiple , 1) ;
		write_temp_login("hbq2zx_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/hb/q2zx.shtml" ;
		to_login(bakUrl);
		return ;
	}
