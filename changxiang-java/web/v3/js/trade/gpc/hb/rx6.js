...
gpc.totalamt = 0 ;
gpc.opentime ;
gpc.jjinfo="选6个号，猜中开奖号全部5个中奖90元" ;
...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("hbrx6lotCode")){
		var numberContent = read_temp_login("hbrx6numberContent") ;
		var pls_bs = read_temp_login("hbrx6_bs") ;
		var pls_qs = read_temp_login("hbrx6_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 6){
		gpc.warn_info("请至少选6个号码");
		return ;
	} else if(selDtBalls.length > 5){
		gpc.warn_info("胆码最多5个");
		return ;
	} else if(selDtBalls.length > 0 && selBalls.length + selDtBalls.length < 7){
		gpc.warn_info("胆码+拖码至少7个");
		return ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("hbrx6lotCode" , "20" , 1) ;
		write_temp_login("hbrx6numberContent" , numberContent , 1) ;
		write_temp_login("hbrx6_play" , gpc.playId , 1) ;
		write_temp_login("hbrx6_bs" , multiple , 1) ;
		write_temp_login("hbrx6_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/hb/rx6.shtml" ;
		to_login(bakUrl);
		return ;
	}
