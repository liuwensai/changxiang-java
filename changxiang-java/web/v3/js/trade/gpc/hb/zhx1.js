...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("hbq1zhxlotCode")){
		var numberContent = read_temp_login("hbq1zhxnumberContent") ;
		var pls_bs = read_temp_login("hbq1zhx_bs") ;
		var pls_qs = read_temp_login("hbq1zhx_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length <rx){
		...
		var s = gpc.calZS(gpc.selball , rx) ;
		gpc.zs = s ;
		gpc.amt = s * 2 ;
...
gpc.calZS = function(sellist , rx){
	...
	var sum_n = 1 ;
	for(var m_n = 1 ; m_n<= selNum-rx ;m_n++){
		sum_n = sum_n*m_n ;
	}
	var sunn = 1 ;
	for(var n = 1 ; n <= rx ; n++){
	...
	t11x5timeCounter("show_time" , false , function(){
		...
		baseAjax("get",url,false,null,"text",function(data){
			...
			if(lot_state.IssueCode != gpc.issuecode){
				...
				var dthtm="<strong>距"+gpc.issuecode+"期截止</strong><strong id=\"show_time\">00分00秒</strong><p class=\"gray mt_5\">选1个号，猜中第1个开奖号，中奖13元</p>" ;
...
function makehtml(json){
	...
	$("#open_code_time_id").html('<strong>距'+info.issuecode+'期截止  </strong><strong id=\"show_time\">00分00秒</strong><p class=\"gray mt_5\">选1个号，猜中第1个开奖号，中奖13元</p>');
	info.back = "" ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>前一： "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("hbq1zhxlotCode" , "1" , 1) ;
		write_temp_login("hbq1zhxnumberContent" , numberContent , 1) ;
		write_temp_login("hbq1zhx_play" , gpc.playId , 1) ;
		write_temp_login("hbq1zhx_bs" , multiple , 1) ;
		write_temp_login("hbq1zhx_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/hb/zhx1.shtml" ;
		to_login(bakUrl);
		return ;
	}
