...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("hbrx3lotCode")){
		var numberContent = read_temp_login("hbrx3numberContent") ;
		var pls_bs = read_temp_login("hbrx3_bs") ;
		var pls_qs = read_temp_login("hbrx3_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("hbrx3lotCode" , "20" , 1) ;
		write_temp_login("hbrx3numberContent" , numberContent , 1) ;
		write_temp_login("hbrx3_play" , gpc.playId , 1) ;
		write_temp_login("hbrx3_bs" , multiple , 1) ;
		write_temp_login("hbrx3_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/hb/rx3.shtml" ;
		to_login(bakUrl);
		return ;
	}
