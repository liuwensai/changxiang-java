...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx2lotCode")){
		var numberContent = read_temp_login("tjrx2numberContent") ;
		var pls_bs = read_temp_login("tjrx2_bs") ;
		var pls_qs = read_temp_login("tjrx2_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 2){
		...
		return ;
	} else if(selBalls.length <rx || selDtBalls.length > drx){
		gpc.warn_info("请至少选2个号码");
		return ;
	} else {
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx2lotCode" , "1" , 1) ;
		write_temp_login("tjrx2numberContent" , numberContent , 1) ;
		write_temp_login("tjrx2_play" , gpc.playId , 1) ;
		write_temp_login("tjrx2_bs" , multiple , 1) ;
		write_temp_login("tjrx2_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx2.shtml" ;
		to_login(bakUrl);
		return ;
	}
