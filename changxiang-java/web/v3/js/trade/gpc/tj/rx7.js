...
gpc.totalamt = 0 ;
gpc.opentime ;
gpc.jjinfo="选7个号，猜中开奖号全部5个中奖26元" ;
...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx7lotCode")){
		var numberContent = read_temp_login("tjrx7numberContent") ;
		var pls_bs = read_temp_login("tjrx7_bs") ;
		var pls_qs = read_temp_login("tjrx7_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 7){
		gpc.warn_info("请至少选7个号码");
		return ;
	} else if(selDtBalls.length > 6){
		gpc.warn_info("胆码最多6个");
		return ;
	} else if(selDtBalls.length > 0 && selBalls.length + selDtBalls.length < 8){
		gpc.warn_info("胆码+拖码至少8个");
		return ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx7lotCode" , "21" , 1) ;
		write_temp_login("tjrx7numberContent" , numberContent , 1) ;
		write_temp_login("tjrx7_play" , gpc.playId , 1) ;
		write_temp_login("tjrx7_bs" , multiple , 1) ;
		write_temp_login("tjrx7_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx7.shtml" ;
		to_login(bakUrl);
		return ;
	}
