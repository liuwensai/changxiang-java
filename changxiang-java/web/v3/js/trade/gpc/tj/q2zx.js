...
gpc.totalamt = 0 ;
gpc.opentime ;
gpc.jjinfo="选3个号，猜中开奖号任意3个中奖19元" ;
...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjq2zxlotCode")){
		var numberContent = read_temp_login("tjq2zxnumberContent") ;
		var pls_bs = read_temp_login("tjq2zx_bs") ;
		var pls_qs = read_temp_login("tjq2zx_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 2){
		gpc.warn_info("请至少选2个号码");
		return ;
	} else if(selDtBalls.length > 1){
		gpc.warn_info("胆码最多1个");
		return ;
	} else if(selDtBalls.length > 0 && selBalls.length + selDtBalls.length < 3){
		gpc.warn_info("胆码+拖码至少3个");
		return ;
		...
		$("#select_code_id").html("请至少选择<span class=\"red\">"+rx+"</span>个号码");
	} else {
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>前二组选： "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjq2zxlotCode" , "21" , 1) ;
		write_temp_login("tjq2zxnumberContent" , numberContent , 1) ;
		write_temp_login("tjq2zx_play" , gpc.playId , 1) ;
		write_temp_login("tjq2zx_bs" , multiple , 1) ;
		write_temp_login("tjq2zx_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/q2zx.shtml" ;
		to_login(bakUrl);
		return ;
	}
