...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx8lotCode")){
		var numberContent = read_temp_login("tjrx8numberContent") ;
		var pls_bs = read_temp_login("tjrx8_bs") ;
		var pls_qs = read_temp_login("tjrx8_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 8){
		gpc.warn_info("请至少选8个号码");
		return ;
	} else if(selDtBalls.length > 7){
		gpc.warn_info("胆码最多7个");
		return ;
	} else if(selDtBalls.length > 0 && selBalls.length + selDtBalls.length < 9){
		gpc.warn_info("胆码+拖码至少9个");
		return ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx8lotCode" , "21" , 1) ;
		write_temp_login("tjrx8numberContent" , numberContent , 1) ;
		write_temp_login("tjrx8_play" , gpc.playId , 1) ;
		write_temp_login("tjrx8_bs" , multiple , 1) ;
		write_temp_login("tjrx8_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx8.shtml" ;
		to_login(bakUrl);
		return ;
	}
