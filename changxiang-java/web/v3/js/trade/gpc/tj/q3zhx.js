...
function initBallObj(){
	...
	$("i[name='q3zhx1']").each(function(i,o){
		selBall1[i] = $(o) ;
...
//				}
//				if(ballObje.selball3[idxvalue-1].context.getAttribute("class")=="tored"){
//					ballObje.selball3[idxvalue-1].context.setAttribute("class","i-redball");
...
//				}
	...
	$("i[name='q3zhx2']").each(function(i,o){
		selBall2[i] = $(o) ;
...
//				}
	...
	$("i[name='q3zhx3']").each(function(i,o){
		selBall3[i] = $(o) ;
		selBall3[i].click(function(){
			var thisp = $(this) ;
			var classValue= thisp.context.getAttribute("class") ;
//			var idxvalue= thisp.context.getAttribute("index") ;
			if(classValue=="i-redball"){
...
$(document).ready(function(){
	...
	$("#tobuy").click(function(){		
		...
		var q2zhx3 = gpc.selball3.join(",") ;
		
		gpc.listCode.push(q2zhx1+"|"+q2zhx2+"|"+q2zhx3) ;
		showselq();
	}) ;
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjq3zhxlotCode")){
		var numberContent = read_temp_login("tjq3zhxnumberContent") ;
		var pls_bs = read_temp_login("tjq3zhx_bs") ;
		var pls_qs = read_temp_login("tjq3zhx_qs") ;
...
function presentSelNum(){
	...
	for (var cc = 1; cc < 12; cc++) {
		...
		baithisp = ballObje.selball3[cc-1].context ;
		baiCls =  baithisp.getAttribute("class") ;
		baiv = baithisp.getAttribute("index") ;
		if(baiCls=="tored"){
			if(baiv < 10){
	...
	if(selBalls1.length < 1 || selBalls2.length <1 || selBalls3.length <1){
		gpc.zs = 0 ;
		gpc.selball1=[] ;
		gpc.selball2=[] ;
		gpc.selball3=[] ;
		...
		gpc.selball3=selBalls3 ;
		var s = gpc.calZS(gpc.selball1,gpc.selball2,gpc.selball3) ;
		gpc.zs = s ;
		gpc.amt = s * 2 ;
		if(s < 1){
			gpc.zs = 0 ;
...
gpc.calZS=function(selList1 , selList2 , selList3){
	var zs = qs(selList1 , selList2 , selList3) ;
	if(zs < 1){
		gpc.zs = 0 ;
		gpc.totalAmt = 0 ;
		gpc.amt = 0 ;
	...
	t11x5timeCounter("show_time" , false , function(){
		...
		baseAjax("get",url,false,null,"text",function(data){
			...
			if(lot_state.IssueCode != gpc.issuecode){
				...
				var dthtm="<strong>距"+gpc.issuecode+"期截止</strong><strong id=\"show_time\">00分00秒</strong><p class=\"gray mt_5\">选3个号，按位猜中前3个开奖号，中奖1170元</p>" ;
...
function initdata(){
	...
	for (var cc = 0; cc < 11; cc++) {
		...
		danObj.setAttribute("class","i-redball");
		danObj = ballObje.selball3[cc].context ;
		danObj.setAttribute("class","i-redball");
	}
...
function makehtml(json){
	...
	$("#open_code_time_id").html('<strong>距'+info.issuecode+'期截止  </strong><strong id=\"show_time\">00分00秒</strong><p class=\"gray mt_5\">选3个号，按位猜中前3个开奖号，中奖1170元</p>');
	info.back = "" ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>前三直选： "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
function showRandom(){
	...
	ballObje.selball3[selRan1[2]].context.setAttribute("class" , "tored") ;
	gpc.selball3.push(selRan1[2]) ;
	presentSelNum();
	//selqList() ;
}
...
function radomone(){
	...
	presentSelNum();
	gpc.listCode.push(gpc.selball1.join(",")+"|"+gpc.selball2.join(",")+"|"+gpc.selball3.join(",")) ;
	showselq();
}
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjq3zhxlotCode" , "1" , 1) ;
		write_temp_login("tjq3zhxnumberContent" , numberContent , 1) ;
		write_temp_login("tjq3zhx_play" , gpc.playId , 1) ;
		write_temp_login("tjq3zhx_bs" , multiple , 1) ;
		write_temp_login("tjq3zhx_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/q3zhx.shtml" ;
		to_login(bakUrl);
		return ;
	}
