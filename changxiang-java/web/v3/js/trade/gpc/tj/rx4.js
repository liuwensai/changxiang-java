...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx4lotCode")){
		var numberContent = read_temp_login("tjrx4numberContent") ;
		var pls_bs = read_temp_login("tjrx4_bs") ;
		var pls_qs = read_temp_login("tjrx4_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx4lotCode" , "21" , 1) ;
		write_temp_login("tjrx4numberContent" , numberContent , 1) ;
		write_temp_login("tjrx4_play" , gpc.playId , 1) ;
		write_temp_login("tjrx4_bs" , multiple , 1) ;
		write_temp_login("tjrx4_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx4.shtml" ;
		to_login(bakUrl);
		return ;
	}
