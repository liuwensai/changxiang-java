...
gpc.totalamt = 0 ;
gpc.opentime ;
gpc.jjinfo="选5个号，猜中开奖号全部5个中奖540元" ;
...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx5lotCode")){
		var numberContent = read_temp_login("tjrx5numberContent") ;
		var pls_bs = read_temp_login("tjrx5_bs") ;
		var pls_qs = read_temp_login("tjrx5_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 5){
		gpc.warn_info("请至少选5个号码");
		return ;
	} else if(selDtBalls.length > 4){
		gpc.warn_info("胆码最多4个");
		return ;
	} else if(selDtBalls.length > 0 && selBalls.length + selDtBalls.length < 6){
		gpc.warn_info("胆码+拖码至少6个");
		return ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx5lotCode" , "21" , 1) ;
		write_temp_login("tjrx5numberContent" , numberContent , 1) ;
		write_temp_login("tjrx5_play" , gpc.playId , 1) ;
		write_temp_login("tjrx5_bs" , multiple , 1) ;
		write_temp_login("tjrx5_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx5.shtml" ;
		to_login(bakUrl);
		return ;
	}
