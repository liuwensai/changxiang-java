...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx6lotCode")){
		var numberContent = read_temp_login("tjrx6numberContent") ;
		var pls_bs = read_temp_login("tjrx6_bs") ;
		var pls_qs = read_temp_login("tjrx6_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx6lotCode" , "21" , 1) ;
		write_temp_login("tjrx6numberContent" , numberContent , 1) ;
		write_temp_login("tjrx6_play" , gpc.playId , 1) ;
		write_temp_login("tjrx6_bs" , multiple , 1) ;
		write_temp_login("tjrx6_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx6.shtml" ;
		to_login(bakUrl);
		return ;
	}
