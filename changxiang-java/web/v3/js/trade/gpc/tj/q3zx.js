...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjq3zxlotCode")){
		var numberContent = read_temp_login("tjq3zxnumberContent") ;
		var pls_bs = read_temp_login("tjq3zx_bs") ;
		var pls_qs = read_temp_login("tjq3zx_qs") ;
...
function presentSelNum(){
	...
	if(selBalls.length < 3){
		gpc.warn_info("请至少选3个号码");
		return ;
	} else if(selDtBalls.length > 2){
		gpc.warn_info("胆码最多2个");
		return ;
	} else if(selDtBalls.length > 0 && selBalls.length + selDtBalls.length < 4){
		gpc.warn_info("胆码+拖码至少4个");
		return ;
...
function showselq(){
	...
	for (var i = liCode.length-1; i >=0; i--) {
		...
		liht+=cont ;
		liht+="</span></var>" ;
		liht+="<p>前三组选： "+getZs(liCode[i])+"注"+getZs(liCode[i])*temp+"元</p>" ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjq3zxlotCode" , "21" , 1) ;
		write_temp_login("tjq3zxnumberContent" , numberContent , 1) ;
		write_temp_login("tjq3zx_play" , gpc.playId , 1) ;
		write_temp_login("tjq3zx_bs" , multiple , 1) ;
		write_temp_login("tjq3zx_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/q3zx.shtml" ;
		to_login(bakUrl);
		return ;
	}
