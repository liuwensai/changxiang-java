...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("tjrx3lotCode")){
		var numberContent = read_temp_login("tjrx3numberContent") ;
		var pls_bs = read_temp_login("tjrx3_bs") ;
		var pls_qs = read_temp_login("tjrx3_qs") ;
...
//	}
	...
	if(!checkLoginByAjax()){		
		...
		write_temp_login("tjrx3lotCode" , "21" , 1) ;
		write_temp_login("tjrx3numberContent" , numberContent , 1) ;
		write_temp_login("tjrx3_play" , gpc.playId , 1) ;
		write_temp_login("tjrx3_bs" , multiple , 1) ;
		write_temp_login("tjrx3_qs" , trackCount , 1) ;
		//登录
		var bakUrl = "/v3shtml/trade/gpc/tj/rx3.shtml" ;
		to_login(bakUrl);
		return ;
	}
