...
$(document).ready(function(){
	...
	//判断是否是登陆后返回
	if(temp_login_exist("zcs_lotCode")){
		sfc.tempData = read_temp_login("zcs_numberContent") ;
	} else {
		sfc.tempData = "" ;
	}
		...
		for(var v = 1 ; v < 7 ; v++){
			...
			if(tempDate[2*(v-1)].indexOf("-")==-1){
				if(tempDate[2*(v-1)].indexOf(",") != -1){
					...
					for(var a = 0 ; a < va.length ; a++){
						$("i[matchidx="+v+"半"+"][v="+va[a]+"]").attr("class","tzb winselect") ;
						sfc.code[v+"半"].push(va[a]);
					}
				} else {
					$("i[matchidx="+v+"半"+"][v="+tempDate[2*(v-1)]+"]").attr("class","tzb winselect") ;
					sfc.code[v+"半"].push(tempDate[2*(v-1)]);
				}
			}
			...
			if(tempDate[2*(v-1)+1].indexOf("-")==-1){
				if(tempDate[2*(v-1)+1].indexOf(",") != -1){
					...
					for(var a = 0 ; a < va.length ; a++){
						$("i[matchidx="+v+"全"+"][v="+va[a]+"]").attr("class","tzb winselect") ;
						sfc.code[v+"全"].push(va[a]);
					}
				} else {
					$("i[matchidx="+v+"全"+"][v="+tempDate[2*(v-1)+1]+"]").attr("class","tzb winselect") ;
					sfc.code[v+"全"].push(tempDate[2*(v-1)+1]);
				}
			}
...
sfc.saleIssue = (function(){
	return function(){
		baseAjax("get" , "/trade/zc/bqc!saleIssue.action?t="+new Date().getTime() , true , null , "json" , function(data){
			var issueData = data;
			var v = 0 ;
			var htm="";
			while(true){
...
sfc.saleMatch=(function(){
	return function(issueCode){
		baseAjax("get" , "/trade/zc/bqc!saleMatch.action?t="+new Date().getTime() , true , null , "text" , function(data){
...
sfc.saleMatchDiv=(function(){
	return function(data , index){
		...
		htm+="</div>";
//		htm+="<div class=\"teams d-box\">";
//		htm+="<div class=\"home b-flex1\">"+data.win+"</div>";
//		htm+="<div class=\"vs1\">"+data.draw+"</div>";
//		htm+="<div class=\"away b-flex1\">"+data.lost+"</div>";
//		htm+="</div>";
//		htm+="<div class=\"jctz d-box\">";
...
//		htm+="</div>";
		...
		htm+="<div class=\"rq1 gray\">半</div>";
		htm+="<label class=\"b-flex\"><i class=\"tzb\" onclick=\"sfc.selectValue(this)\" class=\"tzb\" matchidx=\""+index+"半\" v=\"3\">胜</i></label>";
		htm+="<label class=\"b-flex\"><i class=\"tzb\" onclick=\"sfc.selectValue(this)\" class=\"tzb\" matchidx=\""+index+"半\" v=\"1\">平 </i></label>";
		htm+="<label class=\"b-flex\"><i class=\"tzb\" onclick=\"sfc.selectValue(this)\" class=\"tzb\" matchidx=\""+index+"半\" v=\"0\">负</i></label>";
		htm+="</div>";
		htm+="<div class=\"jctz d-box disun\">";
		htm+="<div class=\"rq1 gray\">全</div>";
		htm+="<label class=\"b-flex\"><i class=\"tzb\" onclick=\"sfc.selectValue(this)\" class=\"tzb\" matchidx=\""+index+"全\" v=\"3\">胜</i></label>";
		htm+="<label class=\"b-flex\"><i class=\"tzb\" onclick=\"sfc.selectValue(this)\" class=\"tzb\" matchidx=\""+index+"全\" v=\"1\">平</i></label>";
		htm+="<label class=\"b-flex\"><i class=\"tzb\" onclick=\"sfc.selectValue(this)\" class=\"tzb\" matchidx=\""+index+"全\" v=\"0\">负</i></label>";
		htm+="</div>";
...
sfc.calCount=(function(){
	return function(selArrays){
		...
		var buycode ="";
		for(var v = 1 ; v<7 ; v++){
			data.zs*=selArrays[v+"半"].length ;
			if(selArrays[v+"半"].length != 0){
				data.cs +=1 ;
			}
			if(v == 1){
				buycode+=""+selArrays[v+"半"].join("") ;
			} else {
				buycode+=","+selArrays[v+"半"].join("") ;
			}
			
			data.zs*=selArrays[v+"全"].length ;
			if(selArrays[v+"全"].length != 0){
				data.cs +=1 ;
			}
			buycode+=","+selArrays[v+"全"].join("") ;
		}
		if(data.cs != 12){
			data.zs = 0 ;
			$("#selectinfo").show();
...
function confirmBuy(isOffline){
	...
	if(!checkLoginByAjax()){		
		...
		for(var v = 1 ; v<7 ; v++){
			if(v == 1){
				tempData+=""+sfc.code[v+"半"].join(",") ;
			} else {
				tempData+="|"+sfc.code[v+"半"].join(",") ;
			}
			tempData+="|"+sfc.code[v+"全"].join(",") ;
		}
		
		write_temp_login("zcs_lotCode" , "7" , 1) ;
		write_temp_login("zcs_numberContent" , tempData , 1) ;
		write_temp_login("zcs_play" , sfc.playId , 1) ;
		write_temp_login("zcs_bs" , multiple , 1) ;
		write_temp_login("zcs_qs" , trackCount , 1) ;
		//登录
//		toAuthLogin();
		var bakUrl = "/v3shtml/trade/zc/zcs.shtml" ;
		to_login(bakUrl);
		return ;
	}
	var trackCount = 1;
