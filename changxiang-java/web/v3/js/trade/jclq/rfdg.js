...
var matchUrl = "/trade/jclq/index!getOneMatchMapForSinAjax.action?playid=10002";
	...
	baseAjax("post",url,false,{pageNumber:pageNumber},"json",function(sdata){
		...
		for(var i = 0;i<data.length;i++){
			...
			for(var k = 0;k<matchdata.length;k++){
				...
				//matchk.copy_writer = matchdata[k].copy_writer;
				...
				//matchk.home_logo = matchdata[k].home_logo;
				//matchk.guest_logo = matchdata[k].guest_logo;
				...
				if(matchdata[k].oddsRfsfg){
					matchk.odds_spfg = matchdata[k].oddsRfsfg;
...
function confirmBuy(){
	...
	if(!checkLoginByAjax()){		
		//保存数据
		write_temp_login("jclqrfdg_lotCode" , jclq.lotid) ;
		write_temp_login("jclqrfdg_gameList" , getListStr(index.gameList)) ;
		write_temp_login("jclqrfdg_ggType" , getListStr(index.ggType) ) ;
		write_temp_login("jclqrfdg_bs" , index.bs ) ;
		//write_temp_login("spf_zs" , spf.totalZs) ;
		var bakUrl = "/v3shtml/trade/jclq/rangfen-dg.shtml" ;
		to_login(bakUrl);
		return ;
	}
	//金额审核
