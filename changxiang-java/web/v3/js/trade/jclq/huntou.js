...
function initMatchData(pageNumber){
	...
	for(var i = 0;i<curData.length;i++){
		...
		for(var k = 0;k<matchdata.length;k++){
			...
			if((matchdata[k].blendMark&jclq.ablePlay_sf)==jclq.ablePlay_sf){
				matchk.ht_sf_sale=1; 
			}else{
				matchk.ht_sf_sale=0;
			}
			if((matchdata[k].blendMark&jclq.ablePlay_rfsf)==jclq.ablePlay_rfsf){
				matchk.ht_rfsf_sale=1; 
			}else{
				matchk.ht_rfsf_sale=0;
			}
			if((matchdata[k].blendMark&jclq.ablePlay_dxf)==jclq.ablePlay_dxf){
				matchk.ht_dxf_sale=1; 
			}else{
				matchk.ht_dxf_sale=0;
			}
			...
			// 设置开售值
			oddsList.ht_sf_sale=matchk.ht_sf_sale;
			oddsList.ht_rfsf_sale=matchk.ht_rfsf_sale;
			oddsList.ht_dxf_sale=matchk.ht_dxf_sale;
			...
			if(matchdata[k].oddsSfg){
				...
				var odds={};
				odds["10001_2"] = oddsArray[0];
				odds["10001_1"] = oddsArray[1];
				oddsList.sf=odds;
			}
			// 让分胜负
			if(matchdata[k].oddsRfsfg){
				var oddsArray = matchdata[k].oddsRfsfg.split(",");
				var odds={};
				odds["10002_2"] = oddsArray[0];
				odds["10002_1"] = oddsArray[1];
				oddsList.rfsf=odds;
			}
			// 大小分
			if(matchdata[k].oddsDxfg){
				...
				var odds={};
				odds["10004_1"] = oddsArray[0];
				odds["10004_2"] = oddsArray[1];
				oddsList.dxf=odds;
			}
			// 胜分差
...
function expandTouzhu(obj){
	...
	// 开售值
	var sfsale = odds.ht_sf_sale;
	var rfsfsale = odds.ht_rfsf_sale;
	var dxfsale = odds.ht_dxf_sale;
	var sfcsale = odds.ht_sfc_sale;
	// 赔率
	var odds_sf = odds.sf;
	var odds_rfsf = odds.rfsf;
	var odds_dxf = odds.dxf;
	...
	if(sfsale==1 && odds_sf){
		obj_sf.show();
		obj_sf.find("i").each(function(i,o){
			var $o = $(o);
			var cvalue = $o.attr("cvalue");
			$o.attr("odds",odds_sf[cvalue]);
			$o.find("span").html(odds_sf[cvalue]);
		});
	}else{
		obj_sf.hide();
	}
	// 让分胜负
	var obj_rfsf = $("li[pl='rfsf']");
	if(rfsfsale==1 && odds_rfsf){
		obj_rfsf.show();
		// 让分值
		if(rq<0){
			obj_rfsf.find("span[rq='rq']").attr("class","green").html("("+rq+")");
		}else{
			obj_rfsf.find("span[rq='rq']").attr("class","red").html("(+"+rq+")");
		}
		obj_rfsf.find("i").each(function(i,o){
			var $o = $(o);
			var cvalue = $o.attr("cvalue");
			$o.attr("odds",odds_rfsf[cvalue]);
			$o.find("span").html(odds_rfsf[cvalue]);
		});
	}else{
		obj_rfsf.hide();
	}
	// 大小分
	var obj_dxf = $("li[pl='dxf']");
	if(dxfsale==1 && odds_dxf){
		obj_dxf.show();
		//预售总分
		obj_dxf.find("span[yszf='yszf']").html(yszf);
		obj_dxf.find("i").each(function(i,o){
			var $o = $(o);
			var cvalue = $o.attr("cvalue");
			$o.attr("odds",odds_dxf[cvalue]);
			$o.find("span").html(odds_dxf[cvalue]);
		});
	}else{
		obj_dxf.hide();
	}
...
function confirmBuy(isOffline){
	...
	if(!checkLoginByAjax()){		
		//保存数据
		write_temp_login("jclqhuntou_lotCode" , jclq.lotid) ;
		write_temp_login("jclqhuntou_gameList" , getListStr(index.gameList)) ;
		write_temp_login("jclqhuntou_ggType" , getListStr(index.ggType) ) ;
		write_temp_login("jclqhuntou_bs" , index.bs ) ;
		//write_temp_login("spf_zs" , spf.totalZs) ;
		var bakUrl = "/v3shtml/trade/jclq/huntou.shtml" ;
		to_login(bakUrl);
		return ;
	}
	//金额审核
...
function doTempBuy(argJSON){
	...
	var betNumber = returnArr[1]; //格式化后的投注号码
	 ...
	 var numberObject = betNumber;//getBuyCode();
     ...
     var lotTypeplay = betPlayId;//jclq.playId;//玩法
     var temp = 2 ;
