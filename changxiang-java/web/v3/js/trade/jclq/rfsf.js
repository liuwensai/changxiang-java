...
function getData(){
	...
	baseAjax("post",matchUrl+t,false,null,"json",function(data){
		...
		for(var i=0; i< r_data.length; i++){
			...
			if((tempObj.blendMark&jclq.ablePlay_rfsf)==jclq.ablePlay_rfsf){
...
function confirmBuy(isOffline){
	...
	if(!checkLoginByAjax()){		
		//保存数据
		write_temp_login("jclqrfsf_lotCode" , jclq.lotid) ;
		write_temp_login("jclqrfsf_gameList" , getListStr(index.gameList)) ;
		write_temp_login("jclqrfsf_ggType" , getListStr(index.ggType) ) ;
		write_temp_login("jclqrfsf_bs" , index.bs ) ;
		//write_temp_login("spf_zs" , spf.totalZs) ;
		var bakUrl = "/v3shtml/trade/jclq/rfsf.shtml" ;
		to_login(bakUrl);
		return ;
	}
	//金额审核
