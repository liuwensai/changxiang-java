...
var tztpl = ["","胜其它","1:0","2:0","2:1","3:0","3:1","3:2","4:0","4:1","4:2","5:0","5:1","5:2","平其它","0:0","1:1","2:2","3:3","负其它","0:1","0:2","1:2","0:3","1:3","2:3","0:4","1:4","2:4","0:5","1:5","2:5"];
...
function getData(){
	...
	baseAjax("post",matchUrl+t,false,null,"json",function(data){
		...
		for(var i=0; i< r_data.length; i++){
			...
			if((tempObj.blendMark&jczq.ablePlay_bf)==jczq.ablePlay_bf){
...
function initMatchData(pageNumber){
	...
	for(var i = 0;i<curData.length;i++){
		...
		for(var k = 0;k<matchdata.length;k++){
			...
			if(matchdata[k].oddsBf){
				matchk.odds_bf = matchdata[k].oddsBf;
			}
...
function backGameList(){
	if(jczq.gameList.length > 0){
		...
		for (var i = 0; i < jczq.gameList.length; i++) {
			...
			var bf_cks = jczq.gameList[i].bf_cks;
			// 选中对阵
			$("i[name='bf_tzinfo_"+cid+"']").html(bf_cks).attr("class","tzb winselect");
		}
	 ...
	 jczq.gameList.each(function(o,i){
		 if(o.cid==cid){
			for(var k = 0 ; k < o.chks.length  ; k++){
				...
				$("i[name='bf_tzinfo_"+cid+"']").removeClass("winselect").html("点击展开投注选项");
			}
			// 删除LI元素
...
function formatGameList(){
	...
	for(var i=0;i<jczq.gameList.length;i++){
		...
		for(var k=0;k<touzhu_nr.length;k++){
			if(touzhu_nr[k]!="90" && touzhu_nr[k]!="99" && touzhu_nr[k]!="09"){
				bfchk.push(touzhu_nr[k].charAt(0)+":"+touzhu_nr[k].charAt(1));
			}else{
				var res =(touzhu_nr[k]=="90"&&"胜其它") || (touzhu_nr[k]=="99"&&"平其它") ||(touzhu_nr[k]=="09"&&"负其它")||"";
				bfchk.push(res);
			}
		}
		var text = bfchk.join(" ");
		var touzhu_str = text.length>14?text.substr(0,14)+"...":text;
		jczq.gameList[i].bf_cks="";
		jczq.gameList[i].bf_cks=touzhu_str;
	}
...
function confirmBuy(isOffline){
	...
	if(!checkLoginByAjax()){		
		//保存数据
		write_temp_login("bf_lotCode" , jczq.lotid) ;
		write_temp_login("bf_gameList" , getListStr(jczq.gameList)) ;
		write_temp_login("bf_ggType" , getListStr(jczq.ggType) ) ;
		write_temp_login("bf_bs" , jczq.bs ) ;
		//write_temp_login("bf_zs" , jczq.totalZs) ;
		var bakUrl = "/v3shtml/trade/jczq/bf.shtml" ;
		to_login(bakUrl);
		return ;
	}
	//金额审核
