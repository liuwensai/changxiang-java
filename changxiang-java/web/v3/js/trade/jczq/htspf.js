...
jczq.ablePlay_rqspf=1; // 让球胜平负开售值
jczq.ablePlay_spf=16;  // 胜平负开售值
	...
	baseAjax("post",matchUrl+t,false,null,"json",function(data){
		...
		for(var i=0; i< r_data.length; i++){
			...
			if(((tempObj.blendMark&jczq.ablePlay_rqspf)==jczq.ablePlay_rqspf)
					|| ((tempObj.blendMark&jczq.ablePlay_spf)==jczq.ablePlay_spf)){
	...
	for(var i = 0;i<curData.length;i++){
		...
		for(var k = 0;k<matchdata.length;k++){
			...
			if((matchdata[k].blendMark&jczq.ablePlay_rqspf)==jczq.ablePlay_rqspf){
				matchk.ht_rqspf_sale=1; // 混投让球胜平负过关开售
			}
			if((matchdata[k].blendMark&jczq.ablePlay_spf)==jczq.ablePlay_spf){
				matchk.ht_spf_sale=5; // 混投胜平负过关开售
			}
			
			if(matchdata[k].oddsSpfg && matchk.ht_spf_sale==5){
...
function xuanhao(obj){
	...
	if($(obj).attr("name")=="touzhu_click_tzb"){
		// 如果没有选择项 就删除胆样式
		$("div[cid='"+buy_parent.attr("cid")+"']").find("i[name='touzhu_click_tzb'][class='tzb winselect']").length==0&&clearDan();
...
function chooseNumber(obj,objele){
	...
	if (k != -1) { 
		...
		var singleInfo = {
			...
			odds:(function(chks){
				var odds={};
				odds.odds_spf=obj.attr("spfA");
				odds.odds_rqspf=obj.attr("rqspfA");
				odds.rqspfSale=obj.attr("rqspfSale");
				odds.spfSale=obj.attr("spfSale");
				return odds;
			})(objele),
...
function setDan(cid){
	...
	// 已设胆数
	var s_danLen = $("ul[id='tobuy_matchList']").find("i[name='dan'][class='jczqsfprangfen-qrpage winselect']").length;
	
	// 设胆
	var dan_class =	$("#tobuy_match_"+cid).find("i[name='dan']");
	jczq.gameList.each(function(o,i){
		if(o.cid==cid){
			if(dan_class.attr("class")=="jczqsfprangfen-qrpage"){
				if(s_danLen >= danLen || danLen==0){
					open_message("设胆数不能超过最小过关数");
					return;
				}
				o.dan="1";
				dan_class.attr("class","jczqsfprangfen-qrpage winselect");
			}else{
				o.dan="0";
				dan_class.attr("class","jczqsfprangfen-qrpage");
			}
		 }
	 });
...
function clearDan(){
	...
	$("i[name='dan']").attr("class","jczqsfprangfen-qrpage");
...
function formatGameList(){
	for(var i=0;i<jczq.gameList.length;i++){
		...
		var rqspf_chs = jczq.gameList[i].choose[1][9001];
		var spf_chs = jczq.gameList[i].choose[0][9005];
		var obj={};
		obj.win_chk  = rqspf_chs.indexOf("3")>=0?"tzb winselect":"tzb";
		obj.draw_chk = rqspf_chs.indexOf("1")>=0?"tzb winselect":"tzb";
		obj.lost_chk = rqspf_chs.indexOf("0")>=0?"tzb winselect":"tzb";
		jczq.gameList[i].rqspf_chs=obj;
		obj={};
		obj.win_chk  = spf_chs.indexOf("3")>=0?"tzb winselect":"tzb";
		obj.draw_chk = spf_chs.indexOf("1")>=0?"tzb winselect":"tzb";
		obj.lost_chk = spf_chs.indexOf("0")>=0?"tzb winselect":"tzb";
		jczq.gameList[i].spf_chs=obj;
		
		var rqspf_odds = jczq.gameList[i].odds.odds_rqspf.split(",");
		var spf_odds = jczq.gameList[i].odds.odds_spf.split(",");
		obj={};
		obj.sodds=rqspf_odds[0];
		obj.podds=rqspf_odds[1];
		obj.fodds=rqspf_odds[2];
		jczq.gameList[i].odds_rqspf = obj ;
		obj={};
		obj.sodds=spf_odds[0];
		obj.podds=spf_odds[1];
		obj.fodds=spf_odds[2];
		jczq.gameList[i].odds_spf = obj ;
	}
...
function confirmBuy(isOffline){
	...
	if(!checkLoginByAjax()){		
		//保存数据
		write_temp_login("htspf_lotCode" , jczq.lotid) ;
		write_temp_login("htspf_gameList" , getListStr(jczq.gameList)) ;
		write_temp_login("htspf_ggType" , getListStr(jczq.ggType) ) ;
		write_temp_login("htspf_bs" , jczq.bs ) ;
		//write_temp_login("htspf_zs" , jczq.totalZs) ;
		var bakUrl = "/v3shtml/trade/jczq/htspf.shtml" ;
		to_login(bakUrl);
		return ;
	}
	//金额审核
