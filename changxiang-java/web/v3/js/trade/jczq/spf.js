...
function getData(){
	...
	baseAjax("post",matchUrl+t,false,null,"json",function(data){
		...
		for(var i=0; i< r_data.length; i++){
			...
			if((tempObj.blendMark&jczq.ablePlay_spf)==jczq.ablePlay_spf){
...
function initMatchData(pageNumber){
	...
	for(var i = 0;i<curData.length;i++){
		...
		for(var k = 0;k<matchdata.length;k++){
			...
			if(matchdata[k].oddsSpfg){
				matchk.odds_spfg = matchdata[k].oddsSpfg;
				var oddsArray = matchdata[k].oddsSpfg.split(",");
...
function xuanhao(obj){
	...
	if($(obj).attr("name")=="touzhu_click_tzb"){
		...
		if(buy_parent.find("i[class='tzb winselect']").length==0){
			// 清除胆
			clearDan();
		}
...
function updateBuyInfo(){
	...
	$('#default_ggtype').text(getGgTypeStr());
...
function confirmBuy(isOffline){
	...
	if(!checkLoginByAjax()){		
		//保存数据
		write_temp_login("spf_lotCode" , jczq.lotid) ;
		write_temp_login("spf_gameList" , getListStr(jczq.gameList)) ;
		write_temp_login("spf_ggType" , getListStr(jczq.ggType) ) ;
		write_temp_login("spf_bs" , jczq.bs ) ;
		//write_temp_login("spf_zs" , jczq.totalZs) ;
		var bakUrl = "/v3shtml/trade/jczq/spf.shtml" ;
		to_login(bakUrl);
		return ;
	}
	//金额审核
