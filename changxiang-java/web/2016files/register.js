...
Reg={
	Email:false,Pass:false,Pass2:false,Yzm:false,_IsLogin:false,
	...
	CheckUser:function(o){//验证用户名注册
		...
		if(len<5||len>18){
			...
			n.parent().find('p').attr('class','u-tips u-tips-err').find('span').html('用户名的长度为5-18个字符（一个汉字2个字符）');
		}else{
			$.post('/RegAction!CheckName.jzh',{name:o,t:local.TimeLong()},function(data){
				if(data=='0'){
					...
					Reg.Email=false;
					n.parent().find('p').attr('class','u-tips u-tips-err').find('span').html('网络连接延时，请重新尝试。');
				}
			});
		}
	},
	...
	Login:function(){//用户登录验证
		...
		if(local.GetLenth(nm)<5||local.GetLenth(nm)>16){
			alert('用户名不正确！');return;
		}
			if (ps == '' || ps == '密  码')
			{
